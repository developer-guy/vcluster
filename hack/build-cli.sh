...
for OS in ${VCLUSTER_BUILD_PLATFORMS[@]}; do
  for ARCH in ${VCLUSTER_BUILD_ARCHS[@]}; do
    ...
    cosign sign-blob --output-signature "${VCLUSTER_ROOT}/release/${NAME}".sha256.sig
--output-certificate "${VCLUSTER_ROOT}/release/${NAME}".sha256.pem "${VCLUSTER_ROOT}/release/${NAME}".sha256
  done
done
